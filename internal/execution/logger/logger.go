// Package logger provides structured-logging and tracing seams for the execution
// orchestrator. It wraps log/slog with a component-scoped factory and defines a
// minimal Tracer interface so distributed tracing can be wired in later without
// the orchestrator depending on any tracing vendor.
package logger
...
// Component tags every log line emitted through a scoped logger, so operators can
// filter execution-orchestrator logs from the rest of the process.
const Component = "execution"

// Named returns a slog.Logger scoped to the given sub-component (e.g. "orchestrator",
// "validation", "registry"). A nil base falls back to slog.Default.
...
// Tracer starts spans for orchestrator operations. A concrete OpenTelemetry
